...
# Các tài nguyên có thể load ngay cả khi không render
...
class CarGameAI:
    ...
    def reset(self):
        ...
        self.car_obs_pos = [
            [self.car_obs_rec_1.x, self.car_obs_rec_1.y],
            [self.car_obs_rec_2.x, self.car_obs_rec_2.y],
            [self.car_obs_rec_3.x, self.car_obs_rec_3.y]
        ]
    ...
    def play_step(self, action):
        ...
        game_over = False
        reward = 0
        if (self._is_collision(self.car_obs_rec_1) or
        ...
        # Điều kiện thắng tuỳ chỉnh
        if self.score >= self.win_score:
            game_over = True
            # có thể cộng thêm chút reward nếu muốn
            return reward, game_over, self.score

            # update ui và clock nếu render
        if self.render:
            self._update_ui()
            self.clock.tick(min(self.score + step_size * 5, 60))
        else:
            # tránh busy-wait quá nóng CPU khi không render
            time.sleep(0.0)
    ...
    def _get_sensor_response(self, sensor):
        return (self.car_obs_rec_1.colliderect(sensor) or
                ...
                self.car_obs_rec_3.colliderect(sensor))
